...
#O código abaixo está completo, só dar import nas coisas abaixo, lembrando que os arquivos nescessarios estão na pasta que eu subi no Onedrive
import os  # Não requer instalação adicional, faz parte da biblioteca padrão do Python
import json
import sys
import glob
import time
import datetime #Essa aqui também é nescessaria, então sem ele dá pau
import threading
import win32print
import win32api
...
import gspread #Pacote para manipular o conteudo do google planilhas
import pandas as pd  # pip install pandas
from openpyxl import load_workbook  # pip install openpyxl
from docxtpl import DocxTemplate  # pip install docxtpl

from oauth2client.service_account import ServiceAccountCredentials #Nescessario para que o meu código se logue no drive.
...
#Confirmar o token valido ou não
def get_client_secrets_file():
    if getattr(sys, 'frozen', False):
        return os.path.join(sys._MEIPASS, 'client_secrets.json')
    else:
...
def autenticar_google_drive():
    ...
    gauth.LoadClientConfigFile(get_client_secrets_file())

    # Se as credenciais não estiverem disponíveis, faça login manual
    if gauth.credentials is None:
        gauth.LocalWebserverAuth()  # Abre o navegador para autenticação
        gauth.SaveCredentialsFile("credenciais_de_login_salvas.json")  # Salva as credenciais

    return gauth


def validar_token(gauth):
    if gauth.access_token_expired:
        print("Token expirado. Atualizando...")
        try:
            gauth.Refresh()  # Tenta atualizar o token usando o refresh token
            gauth.SaveCredentialsFile("credenciais_de_login_salvas.json")  # Salva as novas credenciais
            print("Token atualizado com sucesso.")
        except Exception as e:
            print(f"Erro ao atualizar o token: {e}")
            print("Por favor, faça login manualmente.")
            gauth.LocalWebserverAuth()  # Solicita login manual
            gauth.SaveCredentialsFile("credenciais_de_login_salvas.json")  # Salva as novas credenciais
    else:
        print("Token válido.")
...
def main():
    gauth = autenticar_google_drive()  # Autentica e obtém o objeto GoogleAuth
    validar_token(gauth)  # Valida o token

    # Aqui você pode adicionar o resto do seu código que depende do token válido
    # ...


if __name__ == '__main__':
    main()
#Fim confirmar o token


#Código Zero, copiar as informações da tabela de oficio do google drive


print("Copiando dados da planilha google de oficios para o arquivo Excel. Por favor, aguarde")
...
creds = ServiceAccountCredentials.from_json_keyfile_name("external-comunication-9ae354ab7dc1.json", scope)
...
#Código 5 - criar de fato os documentos de CI e Oficio com a tabela preenchida
...
# Cria a pasta de saída para os documentos do Word
...
mm_texto = meses[mm - 1]  # Obtém o nome do mês correspondente ao número do mês
df["mes"] = mm_texto
...
output_file = r"C:\Users\wesley\PycharmProjects\0 código praticamente completo\CIS e oficios\assinaturas_preenchido.docx"
...
caminho = r"C:\Users\wesley\PycharmProjects\0 código praticamente completo\CIS e oficios"
...
# Função para autenticar e obter acesso ao Google Drive
def autenticar_google_drive():
    gauth = GoogleAuth()

    # Configurações para garantir que o refresh_token seja gerado e salvo
    gauth.settings['oauth_scope'] = ['https://www.googleapis.com/auth/drive']
    gauth.settings['get_refresh_token'] = True
    gauth.settings['oauth_flow_args'] = {'access_type': 'offline', 'prompt': 'consent'}

    # Carrega as credenciais do arquivo 'credenciais_de_login_salvas.json', se existir
    if os.path.exists("credenciais_de_login_salvas.json"):
        gauth.LoadCredentialsFile("credenciais_de_login_salvas.json")
    else:
        gauth.LoadClientConfigFile('client_secrets.json')  # Especifica o caminho para o arquivo 'client_secrets.json'

    # Se as credenciais não existirem ou o token estiver expirado, realiza a autenticação
    if gauth.credentials is None or gauth.access_token_expired:
        gauth.LocalWebserverAuth()  # Abre o navegador para autenticação
        gauth.SaveCredentialsFile("credenciais_de_login_salvas.json")  # Salva as credenciais para reutilização futura
    else:
        gauth.Authorize()  # Se as credenciais estiverem válidas, simplesmente as usa

    # Retorna a instância autenticada do Google Drive
    drive = GoogleDrive(gauth)
    return drive

# Use a função para autenticar e inicializar o Google Drive
drive = autenticar_google_drive()



# Função para encontrar ou criar uma pasta no Google Drive
...
def upload_file_to_drive(drive, local_file_path, folder_id):
    file_name = os.path.basename(local_file_path)
    file_drive = drive.CreateFile({'title': file_name, 'parents': [{'id': folder_id}]})
    file_drive.SetContentFile(local_file_path)
    file_drive.Upload()
    print(f"O arquivo '{file_name}' foi copiado para o Google Drive.")
    time.sleep(1)  # Aguarda 1 segundo após o upload

# Função principal para organizar e fazer o upload dos arquivos
def organize_and_upload_files(drive, local_folder, parent_folder_id_ci, parent_folder_id_of):
    uploads = []  # Lista para armazenar os uploads
    ...
    for file_name in os.listdir(local_folder):
        ...
        month_name = creation_date.strftime('%B')  # Nome do mês em inglês
        month_folder = f"{creation_date.month} {month_translation[month_name]}"  # Nome do mês em português
        day_folder = creation_date.strftime("%d-%m-%Y")

        if file_name.lower().startswith('c'):
            # Caminho para "ci"
            folder_id = get_or_create_folder(drive, parent_folder_id_ci, year_folder)
            folder_id = get_or_create_folder(drive, folder_id, month_folder)
            folder_id = get_or_create_folder(drive, folder_id, day_folder)
        elif file_name.lower().startswith('o'):
            # Caminho para "of"
            folder_id = get_or_create_folder(drive, parent_folder_id_of, year_folder)
            folder_id = get_or_create_folder(drive, folder_id, month_folder)
            folder_id = get_or_create_folder(drive, folder_id, day_folder)
        else:
        ...
        # Adiciona o upload à lista
        uploads.append((drive, local_file_path, folder_id))

    # Faz o upload de todos os arquivos na lista
    for drive, local_file_path, folder_id in uploads:
        upload_file_to_drive(drive, local_file_path, folder_id)



# Caminho da pasta local
local_folder = r"C:\Users\wesley\PycharmProjects\0 código praticamente completo\CIS e oficios"

# IDs das pastas principais no Google Drive
parent_folder_id_ci = '15sk1hfO8nJ8YVxHunGLaF6UQLUNtMfdu'
parent_folder_id_of = '1_q13dGGTktUiPg-saDq7RCGY9EorQKfs'

# Autenticação e inicialização do Google Drive
drive = autenticar_google_drive()

# Organiza e faz o upload dos arquivos
organize_and_upload_files(drive, local_folder, parent_folder_id_ci, parent_folder_id_of)
...
# Código 8 - Excluir arquivos específicos de um diretório
# Caminho do diretório onde os arquivos .docx estão localizados
diretorio = r"C:\Users\wesley\PycharmProjects\0 código praticamente completo\CIS e oficios"

# Padrões de arquivo para localizar arquivos que começam com "o" ou "a"
padroes = [os.path.join(diretorio, '[oO]*.docx'), os.path.join(diretorio, '[aA]*.docx')]

# Iterar sobre os padrões e excluir os arquivos correspondentes
...
print('Processo de exclusão de documentos concluído.')

# Espera para garantir que a exclusão dos arquivos específicos esteja completa
time.sleep(5)  # Ajuste o tempo conforme necessário para garantir que todos os arquivos sejam excluídos
...
# Excluir todos os arquivos restantes do diretório
for arquivo in os.listdir(caminho):
    try:
        os.remove(os.path.join(caminho, arquivo))
        print(f'Arquivo excluído: {os.path.join(caminho, arquivo)}')
    except Exception as e:
        print(f'Erro ao excluir {os.path.join(caminho, arquivo)}: {e}')

print("Todos os arquivos restantes foram excluídos.")



#Código 10 [Em teste] - atualizar as informações na planilha do google drive

# Configuração da autenticação e criação do serviço da API do Google Sheets
SCOPES = ['https://www.googleapis.com/auth/spreadsheets']
creds = Credentials.from_service_account_file('external-comunication-9ae354ab7dc1.json', scopes=SCOPES)
...
for record in df.to_dict(orient="records"):
    ...
    # Caso 2: Se "CI_ou_OF" for "OF" e "giap" tiver um valor específico
        ...
        if found_index:
            ...
            # Exibe a mensagem de registro
            print("Registro criado para o GIAP não encontrado, adicionando nova linha na planilha.")
